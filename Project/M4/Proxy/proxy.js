...
str = "[Proxy-Redis]\n";
fs.appendFile("inventory", str, function(error) {
if (error) {
...
if (err) { console.log("Error in creating instance", err); return; }
  console.log("\n\n----------------Creating instance for Proxy-Redis server-----------------")
          ...
          console.log("ERROR - " + err);
            ...
            if(data.Reservations && data.Reservations[0].Instances) {
              if(data.Reservations[0].Instances.length > 0) {
                ...
                fs.writeFile("proxyIp.txt", instance.PublicIpAddress);

                str1 = "proxy_master : "+ instance.PublicIpAddress;
                fs.writeFile("../Deploy/production/roles/redis_slave/vars/proxy.yml", str1); 

                tcpPortUsed.waitUntilFreeOnHost(22,instance.PublicIpAddress, 200, 10000)
                .then(function() {
                  console.log('Port 22 on '+ instance.PublicIpAddress +' is now free.');
                }, function(err) {
